...
# Day 16
1. Thank you to one of the volunteers for pointing me in the right direction. 
2. I was trying to solve the "y-problem" when the "x-problem" was not running on my local yet!:bang-head: 
3. ON LOCAL: Solved the issue.
4. For me it was changing the ports in the api's. 
5. I had an error that port 5000 was already in use (ie. the flask app).
6. I then set api 1 to 5001, api 2 to 5002. Started both apis up before staring the flask app
7. Going to tackle EC2 instances today 
8. ![image](https://user-images.githubusercontent.com/74470226/199653762-a90920e8-6f25-4d8e-9ae7-7c08216fbbad.png)
...
2. The `First API` consumes the `Second API`. 
   ```  To achieve this: 
        * In the `First API`, I changed the `URL` variable to point to `const URL = "http://Second API - Private IPv4 address:PORT"`. 
          Because comms will now take place inside the `VPC` not the over the internet.
